...
public abstract class AbstractInterfaceConfig extends AbstractMethodConfig {
    ...
    //读取注册信息（注册中心配置）     <dubbo:registry address="zookeeper://127.0.0.1:2181"/>
        ...
        if (registries != null && !registries.isEmpty()) {
            for (RegistryConfig config : registries) {
                ...
                if (!RegistryConfig.NO_AVAILABLE.equalsIgnoreCase(address)) {
                    ...
                    //将application与register 的配置 保存到map 中
                    ...
                    for (URL url : urls) {
                        ...
                        //如果是服务提供者，并且向注册 注册中心 注册服务，则添加到registryList 列表中
                        //如果不是服务提供者，并且向 注册中心 订阅，则添加到registryList 列表中
    ...
    protected void checkInterfaceAndMethods(Class<?> interfaceClass, List<MethodConfig> methods) {
        ...
        if (methods != null && !methods.isEmpty()) {
            //遍历MethodConfig配置<dubbo:method 中的配置
            for (MethodConfig methodBean : methods) {
                //方法配置 设置Service 是 <dubbo:service interface="com.jerry.dubbo.spring.service.DemoService"
