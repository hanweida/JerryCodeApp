...
/**
 * redis常用的操作
 * @Class Name RedisTest
 * @Author wushuo
 * @Create In Apr 17, 2012
 */
...
//			is = SyncToRedis.class.getClassLoader().getResourceAsStream(SYSTEM_CONFIG_FILE);
	...
	private static void initialPool(Properties props) {
		...
		config.setMaxActive(Integer.parseInt(props.getProperty("redis_maxActive")));
		config.setMaxIdle(Integer.parseInt(props.getProperty("redis_setMaxIdle")));
		config.setMaxWait(Long.valueOf(props.getProperty("redis_maxWait")));
		config.setTestOnBorrow(true);
		config.setTestOnReturn(true);
		config.setTestWhileIdle(true);
        try {
            jedisPool = new JedisPool(config, props.getProperty("redis_host"), Integer.parseInt(props.getProperty("redis_port")), Integer.parseInt(props.getProperty("redis_timeout")), props.getProperty("redis_password"));
            jedis = jedisPool.getResource();
        } catch (JedisConnectionException e){
            jedisPool = new JedisPool(config, props.getProperty("redis_host"), Integer.parseInt(props.getProperty("redis_port")), Integer.parseInt(props.getProperty("redis_timeout")));
        }
	}
